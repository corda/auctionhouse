...
class AuctionSettleFlowTests {
    ...
    fun flowReturnsCorrectlyFormedSignedTransaction() {
        ...
        issueCash(bidder, 5.POUNDS)
        val inputAuction = stx.tx.outputs.single().data as AuctionState
        val wait = inputAuction.expiry.toEpochMilli() - Instant.now().toEpochMilli()
        Thread.sleep(max(wait, 0L))
        val flow = AuctionSettleFlow(StateRef(stx.id, 0))
        val futureSettle = bidder.startFlow(flow)
        mockNetwork.runNetwork()
        val settleResult = futureSettle.getOrThrow()
        bidder.transaction {
            val ledgerTx = settleResult.toLedgerTransaction(bidder.services, false)
            assert(ledgerTx.inputs.size == 3)
            assert(ledgerTx.outputs.size == 3)
            ...
            assertEquals(
                    outputAuctionItem,
                    inputAuctionItem.transfer(bidder.info.singleIdentity()))
            val outputCashSum = ledgerTx.outputs
                    .map { it.data }
                    .filterIsInstance<Cash.State>()
                    .filter { it.owner == seller.info.chooseIdentityAndCert().party }
                    .sumCash()
                    .withoutIssuer()
            assertEquals(outputCashSum, inputAuction.price)
            val auctionCommand = ledgerTx.commands.requireSingleCommand<AuctionContract.Commands>()
            assert(auctionCommand.value == AuctionContract.Commands.Settle())
            val auctionItemCommand = ledgerTx.commands.requireSingleCommand<AuctionItemContract.Commands>()
            assert(auctionItemCommand.value == AuctionItemContract.Commands.Transfer())
    ...
    fun flowWithoutBidderReturnsCorrectlyFormedSignedTransaction() {
        val seller = a
        val stx = listAuction(seller, 1.POUNDS, Instant.now().plusSeconds(10))
        val inputAuction = stx.tx.outputsOfType(AuctionState::class.java).single()
        val wait = inputAuction.expiry.toEpochMilli() - Instant.now().toEpochMilli()
        Thread.sleep(max(wait, 0L))
        val flow = AuctionSettleFlow(StateRef(stx.id, 0))
        val futureSettle = seller.startFlow(flow)
        mockNetwork.runNetwork()
        val settleResult = futureSettle.getOrThrow()
        seller.transaction {
            val ledgerTx = settleResult.toLedgerTransaction(seller.services, false)
    ...
    @Test
    fun settleFlowCannotBeRunBySellerIfBidderExists() {
        val seller = a
        val bidder = b
        val auctionId = listAuction(seller, 1.POUNDS, Instant.now().plusSeconds(30)).tx.outputsOfType<AuctionState>().single().linearId
        val futureBid = bidder.startFlow(AuctionBidFlow(auctionId, 2.POUNDS))
    ...
    @Test
    fun settleFlowCannotBeRunByNeitherSellerOrBidder() {
        val seller = a
        val bidder = b
        val auctionId = listAuction(seller, 1.POUNDS, Instant.now().plusSeconds(30)).tx.outputsOfType<AuctionState>().single().linearId
        val futureBid = bidder.startFlow(AuctionBidFlow(auctionId, 2.POUNDS))
